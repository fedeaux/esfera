...
PDFTron.WebViewer=function WebViewer(options,element){this.options=$.extend(true,{},this.defaultOptions,options);if(typeof options.path!=="undefined"){var lastCharIndex=this.options.path.length-1;if(lastCharIndex>0&&this.options.path[lastCharIndex]!=="/")this.options.path+="/";this.options.flash=this.options.path+this.options.flash;this.options.html5=this.options.path+this.options.html5;this.options.html5Mobile=this.options.path+this.options.html5Mobile;this.options.silverlight=this.options.path+
this.options.silverlight}this.element=element;$(this.element).css("overflow","hidden");var me=this;if(typeof this.options.initialDoc!=="undefined"){var docPath=this._correctRelativePath(this.options.initialDoc);docPath=encodeURIComponent(docPath);this.options.initialDoc=docPath;this._create()}else if(typeof this.options.cloudApiId!=="undefined")$.get("https://api.pdftron.com/v2/download/"+this.options.cloudApiId+"?type=xod&redirect=false",function(data){if(typeof data.url==="undefined")me.loadErrorPage();
else{me.options.initialDoc=encodeURIComponent(data.url);me._create()}},"json").error(function(){me.loadErrorPage()})};
PDFTron.WebViewer.prototype={version:"1.5.0",defaultOptions:{type:"html5,silverlight,html5Mobile,flash",path:"",flash:"flash/ReaderControl.swf",html5:"html5/ReaderControl.html",html5Mobile:"html5/MobileReaderControl.html",silverlight:"silverlight/ReaderControl.xap",initialDoc:undefined,cloudApiId:undefined,enableAnnotations:false,enableOfflineMode:false,enableReadOnlyMode:false,errorPage:undefined,serverUrl:undefined,documentId:undefined,streaming:false,config:undefined,encryption:undefined,showSilverlightControls:true,
silverlightObjectParams:{}},_create:function(){var me=this;me.rcId=((1+Math.random())*65536|0).toString(16).substring(1);if(typeof this._trigger=="undefined")this._trigger=function(type,event,data){data=data||{};event=$.Event(event);event.type=type;event.target=$(this.element)[0];event.data=data;$(this.element).trigger(event,data)};var viewers=this.options.type.replace(" ","").split(",");if(viewers.length<1)viewers[0]="html5";me._flashVer="10.2";me._flashFABridgeId="ReaderControl";me._silverlightVer=
"4.0";me._createViewer(viewers)},_createViewer:function(viewers){var me=this;me.selectedType="none";if(this.isMobileDevice()){viewers=Array("html5Mobile");var newLocation=this.options.html5Mobile+this._getHTML5OptionsURL();me.selectedType="html5Mobile";window.location=newLocation;return}var allowHTML5=false;var allowSilverlight=false;var allowFlash=false;for(var i=0;i<viewers.length;i++){if(viewers[i].toLowerCase()==="html5mobile"){allowHTML5=true;if(me._testHTML5()){me.selectedType="html5Mobile";
break}}if(viewers[i].toLowerCase()==="html5"){allowHTML5=true;if(me._testHTML5()){me.selectedType="html5";break}}else if(viewers[i].toLowerCase()==="silverlight"){allowSilverlight=true;if(me._testSilverlight(me._silverlightVer)){me.selectedType="silverlight";break}}else if(viewers[i].toLowerCase()==="flash"){allowFlash=true;if(me._testFlash(me._flashVer)){me.selectedType="flash";break}}}if(me.selectedType==="html5")me._createHTML5();else if(me.selectedType==="html5Mobile")me._createHTML5Mobile();
else if(me.selectedType==="silverlight")me._createSilverlight();else if(me.selectedType==="flash")me._createFlash();else{var supportErrorArray=new Array("Please");if(allowHTML5){supportErrorArray.push("use an HTML5 compatible browser");if(allowSilverlight||allowFlash)supportErrorArray.push("or")}if(allowSilverlight||allowFlash){var runtimesStr="("+(allowSilverlight?"Silverlight":"")+(allowSilverlight&&allowFlash?" or ":"")+(allowFlash?"Flash":"")+")";supportErrorArray.push("install the necessary runtime "+
runtimesStr)}$(me.element).append('<div id="webviewer-browser-unsupported">'+supportErrorArray.join(" ")+".</div>")}},_viewerLoaded:function(iframe){var me=this;me._trigger("ready");var viewerWindow=iframe.contentWindow;if(typeof me.options.encryption!=="undefined"){var decrypt=viewerWindow.CoreControls.Encryption.Decrypt;var doc=decodeURIComponent(me.options.initialDoc);var streaming=me.options.streaming;viewerWindow.ControlUtils.byteRangeCheck(function(status){if(status===200)streaming=true;me.instance.loadDocument(doc,
streaming,decrypt,me.options.encryption)},function(){streaming=true;me.instance.loadDocument(doc,streaming,decrypt,me.options.encryption)})}if(me.instance.docViewer.GetDocument()==null)viewerWindow.$(iframe.contentDocument).bind("documentLoaded",function(event){me._trigger(event.type)});else me._trigger("documentLoaded");viewerWindow.$(iframe.contentDocument).bind("displayModeChanged zoomChanged pageChanged fitModeChanged toolModeChanged",function(event,data){me._trigger(event.type,null,data)})},
_getHTML5OptionsURL:function(){var options=this.options;var url="";if(typeof options.initialDoc!=="undefined"){url+="?d="+options.initialDoc;if(options.streaming)url+="&streaming=true";if(options.encryption)url+="&auto_load=false";if(options.enableAnnotations)url+="&a=1";if(typeof options.serverUrl!=="undefined"){var serverUrl=this._correctRelativePath(options.serverUrl);serverUrl=encodeURIComponent(serverUrl);url+="&server_url="+serverUrl}if(typeof options.documentId!=="undefined")url+="&did="+options.documentId;
if(typeof options.config!=="undefined"){var config=this._correctRelativePath(options.config);config=encodeURIComponent(config);url+="&config="+config}if(options.enableOfflineMode)url+="&offline=1";if(options.enableReadOnlyMode)url+="&readonly=1";if(typeof options.annotationUser!=="undefined")url+="&user="+options.annotationUser;if(typeof options.annotationAdmin!=="undefined")url+="&admin="+(options.annotationAdmin?1:0);if(typeof options.custom!=="undefined")url+="&custom="+encodeURIComponent(options.custom)}return url},
_createHTML5:function(){var me=this;var iframeSource=this.options.html5+this._getHTML5OptionsURL();var $rcFrame=$(document.createElement("iframe"));$rcFrame.attr({id:this.rcId,src:iframeSource,frameborder:0,width:"100%",height:"100%",allowFullScreen:true,webkitallowfullscreen:true,mozallowfullscreen:true});var outerWindow=window;$rcFrame.load(function(){me.instance=this.contentWindow.readerControl;outerWindow.CoreControls=this.contentWindow.CoreControls;var iframe=this;if(typeof me.instance==="undefined")this.contentWindow.$(this.contentDocument).bind("viewerLoaded",
function(event){me.instance=iframe.contentWindow.readerControl;me._viewerLoaded(iframe)});else me._viewerLoaded(iframe)});$(this.element).append($rcFrame);return $rcFrame},_createHTML5Mobile:function(){var me=this;var iframeSource=this.options.html5Mobile+this._getHTML5OptionsURL();var $rcFrame=$(document.createElement("iframe"));$rcFrame.attr({id:this.rcId,src:iframeSource,frameborder:0});$rcFrame.css("width","100%").css("height","100%");$rcFrame.load(function(){me.instance=this.contentWindow.readerControl;
var iframe=this;if(typeof me.instance==="undefined")this.contentWindow.$(this.contentDocument).bind("viewerLoaded",function(event){me.instance=iframe.contentWindow.readerControl;me._viewerLoaded(iframe)});else me._viewerLoaded(iframe)});$(this.element).append($rcFrame);return $rcFrame},_getSilverlightInitParam:function(){var options=this.options;var initParam="";if(options.showSilverlightControls)initParam+="UseJavaScript=false";else initParam+="UseJavaScript=true";if(typeof options.initialDoc!==
"undefined"){initParam+=",DocumentUri="+function decode(str){return unescape(str.replace(/\+/g," "))}(options.initialDoc);if(options.streaming)initParam+=",Streaming=true";else initParam+=",Streaming=false";if(options.enableAnnotations)initParam+=",a=true";if(typeof options.serverUrl!=="undefined"){var serverUrl=this._correctRelativePath(options.serverUrl);serverUrl=encodeURIComponent(serverUrl);initParam+=",server_url="+serverUrl}if(typeof options.documentId!=="undefined")initParam+=",did="+options.documentId;
if(typeof options.config!=="undefined")initParam+=",config="+options.config;if(options.enableOfflineMode)initParam+=",offline=true";if(options.enableReadOnlyMode)initParam+=",readonly=true";if(typeof options.annotationUser!=="undefined")initParam+=",user="+options.annotationUser;if(typeof options.annotationAdmin!=="undefined")if(options.annotationAdmin)initParam+=",admin=true";else initParam+=",admin=false";if(typeof options.custom!=="undefined")initParam+=",custom="+encodeURIComponent(options.custom)}return initParam},
_createSilverlight:function(){var me=this;var options=this.options;var initParam=this._getSilverlightInitParam();var objectParams={"source":options.silverlight,"background":"white","minRuntimeVersion":"4.0.50401.0","autoUpgrade":"true"};$.extend(objectParams,options.silverlightObjectParams);var objectElement=Silverlight.createObject(options.silverlight,null,me.rcId,objectParams,{onLoad:function(sender,args){me.instance=sender.Content.ReaderControl;me.instance.EnableAnnotations=options.enableAnnotations;
me.instance.onPropertyChanged=function(sender2,args2){switch(args2.name){case "CurrentPageNumber":me._trigger("pageChanged",null,{pageNumber:args2.pageNumber});break;case "DisplayMode":me._trigger("displayModeChanged");break;case "Zoom":me._trigger("zoomChanged");break;case "FitMode":me._trigger("fitModeChanged");break;case "ToolMode":me._trigger("toolModeChanged");break}};me._trigger("ready");me.instance.onDocumentLoaded=function(sender3,args3){me._trigger("documentLoaded")}},onError:function(){console.error("Silverlight onError")}},
initParam,null);$(objectElement).attr({"width":"100%","height":"100%"}).appendTo(me.element)},_createFlash:function(){var me=this;var options=this.options;var swfUrlStr=options.flash;var flashObj=document.createElement("object");flashObj.setAttribute("id",me.rcId);$(flashObj).appendTo(me.element);var xiSwfUrl="";var flashvars={};flashvars.bridgeName=me._flashFABridgeId;if(options.initialDoc!=undefined)flashvars.d=options.initialDoc;if(options.streaming!=undefined)flashvars.streaming=options.streaming;
flashvars.NonStreamingMode="AJAX";var params={};var attributes={};attributes.id=me._flashFABridgeId;attributes.name=me._flashFABridgeId;if(typeof options.encryption!=="undefined")createHttpAjaxPartRetriever(me._flashFABridgeId,decodeURIComponent(flashvars.d),window.CoreControls.Encryption.DecryptSynchronous,options.encryption);swfobject.embedSWF(swfUrlStr,me.rcId,"100%","100%",me._flashVer,xiSwfUrl,flashvars,params,attributes,function(e){if(e.success)FABridge.addInitializationCallback(me._flashFABridgeId,
function(e){me.instance=FABridge.ReaderControl.root();var _cb=function(event){me._trigger(event.getType())};var _errorcb=function(event){console.error(event.getText())};me.instance.addEventListener("zoomChanged",_cb);me.instance.addEventListener("pageChanged",_cb);me.instance.addEventListener("displayModeChanged",_cb);me.instance.addEventListener("toolModeChanged",_cb);me.instance.addEventListener("errorEvent",_errorcb);me._trigger("ready");if(me.instance.IsDocLoaded())me._trigger("documentLoaded");
else me.instance.addEventListener("documentLoaded",function(event){me._trigger("documentLoaded")})});else console.error("swfobject.embedSWF failed")})},_init:function(){},getInstance:function(){return this.instance},loadErrorPage:function(){if(typeof this.options.errorPage=="undefined")$(this.element).append("<h2 style='text-align:center;margin-top:100px'>We're sorry, an error has occurred.</h2>");else $(this.element).load(this.options.errorPage)},getShowSideWindow:function(){if(this.selectedType===
"html5"||this.selectedType==="html5Mobile")return this.getInstance().getShowSideWindow();else if(this.selectedType==="silverlight"||this.selectedType==="flash")return this.getInstance().GetShowSideWindow()},setShowSideWindow:function(value){if(this.selectedType==="html5"||this.selectedType==="html5Mobile")this.getInstance().setShowSideWindow(value);else if(this.selectedType==="silverlight")this.getInstance().SetShowSideWindow(value);else if(this.selectedType==="flash")this.getInstance().SetShowSideWindow(value==
"true")},getCurrentPageNumber:function(){if(this.selectedType==="html5"||this.selectedType==="html5Mobile")return this.getInstance().getCurrentPageNumber();else if(this.selectedType==="flash")return this.getInstance().GetCurrentPage();else if(this.selectedType==="silverlight")return this.instance.CurrentPageNumber},setCurrentPageNumber:function(pageNumber){if(this.selectedType==="html5"||this.selectedType==="html5Mobile")this.getInstance().setCurrentPageNumber(pageNumber);else if(this.selectedType===
"flash")this.getInstance().SetCurrentPage(pageNumber);else if(this.selectedType==="silverlight")this.getInstance().CurrentPageNumber=pageNumber},getPageCount:function(){if(this.selectedType==="html5"||this.selectedType==="html5Mobile")return this.getInstance().getPageCount();else if(this.selectedType==="flash")return this.getInstance().GetPageCount();else if(this.selectedType==="silverlight")return this.getInstance().PageCount},getZoomLevel:function(){if(this.selectedType==="html5"||this.selectedType===
"html5Mobile")return this.getInstance().getZoomLevel();else if(this.selectedType==="flash")return this.getInstance().GetZoomLevel();else if(this.selectedType==="silverlight")return this.getInstance().ZoomLevel},setZoomLevel:function(zoomLevel){if(this.selectedType==="html5"||this.selectedType==="html5Mobile")this.getInstance().setZoomLevel(zoomLevel);else if(this.selectedType==="flash")this.getInstance().SetZoomLevel(zoomLevel);else if(this.selectedType==="silverlight")this.getInstance().ZoomLevel=
zoomLevel},rotateClockwise:function(){if(this.selectedType==="html5"||this.selectedType==="html5Mobile")this.getInstance().rotateClockwise();else if(this.selectedType==="flash")this.getInstance().RotateClockwise();else if(this.selectedType==="silverlight")this.getInstance().RotateClockwise()},rotateCounterClockwise:function(){if(this.selectedType==="html5"||this.selectedType==="html5Mobile")this.getInstance().rotateCounterClockwise();else if(this.selectedType==="flash")this.getInstance().RotateCounterClockwise();
else if(this.selectedType==="silverlight")this.getInstance().RotateCounterClockwise()},getLayoutMode:function(){if(this.selectedType==="html5"){var layoutMode=this.getInstance().getLayoutMode();var displayModes=CoreControls.DisplayModes;if(layoutMode===displayModes.Single)return PDFTron.WebViewer.LayoutMode.Single;else if(layoutMode===displayModes.Continuous)return PDFTron.WebViewer.LayoutMode.Continuous;else if(layoutMode===displayModes.Facing)return PDFTron.WebViewer.LayoutMode.Facing;else if(layoutMode===
displayModes.FacingContinuous)return PDFTron.WebViewer.LayoutMode.FacingContinuous;else if(layoutMode===displayModes.Cover)return PDFTron.WebViewer.LayoutMode.FacingCoverContinuous;else if(layoutMode===displayModes.CoverFacing)return PDFTron.WebViewer.LayoutMode.FacingCover;else return undefined}else if(this.selectedType==="html5Mobile")this.getInstance().getLayoutMode();else if(this.selectedType==="silverlight"||this.selectedType==="flash")return this.getInstance().GetLayoutMode()},setLayoutMode:function(layoutMode){if(this.selectedType===
"html5"){var displayModes=CoreControls.DisplayModes;var displayMode=displayModes.Continuous;if(layoutMode===PDFTron.WebViewer.LayoutMode.Single)displayMode=displayModes.Single;else if(layoutMode===PDFTron.WebViewer.LayoutMode.Continuous)displayMode=displayModes.Continuous;else if(layoutMode===PDFTron.WebViewer.LayoutMode.Facing)displayMode=displayModes.Facing;else if(layoutMode===PDFTron.WebViewer.LayoutMode.FacingContinuous)displayMode=displayModes.FacingContinuous;else if(layoutMode===PDFTron.WebViewer.LayoutMode.FacingCover)displayMode=
displayModes.CoverFacing;else if(layoutMode===PDFTron.WebViewer.LayoutMode.FacingCoverContinuous)displayMode=displayModes.Cover;this.getInstance().setLayoutMode(displayMode)}else if(this.selectedType==="html5Mobile")this.getInstance().setLayoutMode(layoutMode);else if(this.selectedType==="silverlight"||this.selectedType==="flash")this.getInstance().SetLayoutMode(layoutMode)},getToolMode:function(){if(this.selectedType==="html5"){var toolMode=this.getInstance().getToolMode();var toolModes=this.getInstance().docViewer.ToolModes;
if(toolMode instanceof toolModes.Pan)return PDFTron.WebViewer.ToolMode.Pan;else if(toolMode instanceof toolModes.TextSelect)return PDFTron.WebViewer.ToolMode.TextSelect;else if(toolMode instanceof toolModes.AnnotationEdit)return PDFTron.WebViewer.ToolMode.AnnotationEdit;else if(toolMode instanceof toolModes.EllipseCreate)return PDFTron.WebViewer.ToolMode.AnnotationCreateEllipse;else if(toolMode instanceof toolModes.FreeHandCreate)return PDFTron.WebViewer.ToolMode.AnnotationCreateFreeHand;else if(toolMode instanceof
toolModes.LineCreate)return PDFTron.WebViewer.ToolMode.AnnotationCreateLine;else if(toolMode instanceof toolModes.RectangleCreate)return PDFTron.WebViewer.ToolMode.AnnotationCreateRectangle;else if(toolMode instanceof toolModes.StickyCreate)return PDFTron.WebViewer.ToolMode.AnnotationCreateSticky;else if(toolMode instanceof toolModes.TextHighlightCreate)return PDFTron.WebViewer.ToolMode.AnnotationCreateTextHighlight;else if(toolMode instanceof toolModes.TextStrikeoutCreate)return PDFTron.WebViewer.ToolMode.AnnotationCreateTextStrikeout;
else if(toolMode instanceof toolModes.TextUnderlineCreate)return PDFTron.WebViewer.ToolMode.AnnotationCreateTextUnderline;else if(toolMode instanceof toolModes.CustomLineCreate)return PDFTron.WebViewer.ToolMode.AnnotationCreateCustom;else return null}else if(this.selectedType==="html5Mobile")return this.getInstance().getToolMode();else if(this.selectedType==="silverlight"||this.selectedType==="flash")return this.getInstance().GetToolMode()},setToolMode:function(toolMode){if(this.selectedType==="html5"){var toolModes=
this.getInstance().docViewer.ToolModes;var modeToSet=toolModes.Pan;if(toolMode===PDFTron.WebViewer.ToolMode.Pan)modeToSet=toolModes.Pan;else if(toolMode===PDFTron.WebViewer.ToolMode.TextSelect)modeToSet=toolModes.TextSelect;else if(toolMode===PDFTron.WebViewer.ToolMode.AnnotationEdit)modeToSet=toolModes.AnnotationEdit;else if(toolMode===PDFTron.WebViewer.ToolMode.AnnotationCreateEllipse)modeToSet=toolModes.EllipseCreate;else if(toolMode===PDFTron.WebViewer.ToolMode.AnnotationCreateFreeHand)modeToSet=
toolModes.FreeHandCreate;else if(toolMode===PDFTron.WebViewer.ToolMode.AnnotationCreateLine)modeToSet=toolModes.LineCreate;else if(toolMode===PDFTron.WebViewer.ToolMode.AnnotationCreateRectangle)modeToSet=toolModes.RectangleCreate;else if(toolMode===PDFTron.WebViewer.ToolMode.AnnotationCreateSticky)modeToSet=toolModes.StickyCreate;else if(toolMode===PDFTron.WebViewer.ToolMode.AnnotationCreateTextHighlight)modeToSet=toolModes.TextHighlightCreate;else if(toolMode===PDFTron.WebViewer.ToolMode.AnnotationCreateTextStrikeout)modeToSet=
toolModes.TextStrikeoutCreate;else if(toolMode===PDFTron.WebViewer.ToolMode.AnnotationCreateTextUnderline)modeToSet=toolModes.TextUnderlineCreate;else if(toolMode===PDFTron.WebViewer.ToolMode.AnnotationCreateCustom)modeToSet=toolModes.CustomLineCreate;this.getInstance().setToolMode(modeToSet)}else if(this.selectedType==="html5Mobile")this.getInstance().setToolMode(toolMode);else if(this.selectedType==="silverlight"||this.selectedType==="flash")this.getInstance().SetToolMode(toolMode)},fitWidth:function(){if(this.selectedType===
"html5"||this.selectedType==="html5Mobile")this.getInstance().fitWidth();else if(this.selectedType==="flash")this.getInstance().FitWidth();else if(this.selectedType==="silverlight")this.getInstance().FitWidth()},fitHeight:function(){if(this.selectedType==="html5"||this.selectedType==="html5Mobile")this.getInstance().fitHeight();else if(this.selectedType==="flash")console.warn("Unsupported method fitHeight.");else if(this.selectedType==="silverlight")this.getInstance().FitHeight()},fitPage:function(){if(this.selectedType===
"html5"||this.selectedType==="html5Mobile")this.getInstance().fitPage();else if(this.selectedType==="flash")this.getInstance().FitPage();else if(this.selectedType==="silverlight")this.getInstance().FitPage()},zoom:function(){if(this.selectedType==="html5"||this.selectedType==="html5Mobile")this.getInstance().fitZoom();else if(this.selectedType==="flash")this.getInstance().FitZoom();else if(this.selectedType==="silverlight")this.getInstance().Zoom()},goToFirstPage:function(){if(this.selectedType===
"html5"||this.selectedType==="html5Mobile")this.getInstance().goToFirstPage();else if(this.selectedType==="flash")this.getInstance().GoToFirstPage();else if(this.selectedType==="silverlight")this.getInstance().CurrentPageNumber=1},goToLastPage:function(){if(this.selectedType==="html5"||this.selectedType==="html5Mobile")this.getInstance().goToLastPage();else if(this.selectedType==="flash")this.getInstance().GoToLastPage();else if(this.selectedType==="silverlight"){var totalPages=this.getInstance().PageCount;
this.getInstance().CurrentPageNumber=totalPages}},goToNextPage:function(){if(this.selectedType==="html5"||this.selectedType==="html5Mobile")this.getInstance().goToNextPage();else if(this.selectedType==="flash")this.getInstance().GoToNextPage();else if(this.selectedType==="silverlight"){var currentPage=this.getInstance().CurrentPageNumber;if(currentPage<=0)return;currentPage=currentPage+1;this.getInstance().CurrentPageNumber=currentPage}},goToPrevPage:function(){if(this.selectedType==="html5"||this.selectedType===
"html5Mobile")this.getInstance().goToPrevPage();else if(this.selectedType==="flash")this.getInstance().GoToPrevPage();else if(this.selectedType==="silverlight"){var currentPage=this.getInstance().CurrentPageNumber;if(currentPage<=1)return;currentPage=currentPage-1;this.getInstance().CurrentPageNumber=currentPage}},loadDocument:function(url){if(this.selectedType==="html5"||this.selectedType==="html5Mobile")this.getInstance().loadDocument(this._correctRelativePath(url),this.options.streaming);else this.getInstance().LoadDocument(this._correctRelativePath(url),
this.options.streaming)},searchText:function(pattern,searchModes){var mode=0;var modes=searchModes;if(typeof modes=="string")modes=searchModes.split(",");if(typeof searchModes!="undefined")for(var i=0;i<modes.length;i++)if(searchModes[i]==="CaseSensitive")mode+=1;else if(searchModes[i]==="WholeWord")mode+=2;else if(searchModes[i]==="SearchUp")mode+=4;else if(searchModes[i]==="PageStop")mode+=8;else if(searchModes[i]==="ProvideQuads")mode+=16;else if(searchModes[i]==="AmbientString")mode+=32;if(this.selectedType===
"html5"||this.selectedType==="html5Mobile")if(typeof searchModes=="undefined")this.getInstance().searchText(pattern);else this.getInstance().searchText(pattern,mode);else if(this.selectedType==="silverlight"){var modeString=$.isArray(searchModes)?searchModes.join(","):searchModes;this.getInstance().SearchText(pattern,modeString)}else if(this.selectedType==="flash")if(typeof searchModes=="undefined")this.getInstance().SearchText(pattern);else this.getInstance().SearchText(pattern,mode)},setOnPageChangeCallback:function(callback){if(this.selectedType===
"html5"||this.selectedType==="html5Mobile")this.getInstance().setOnPageChangeCallback(callback);else if(this.selectedType==="flash")alert("Not yet supported");else if(this.selectedType==="silverlight")this.getInstance().OnPageChangeCallback=callback},setOnPageZoomCallback:function(callback){if(this.selectedType==="html5"||this.selectedType==="html5Mobile")this.getInstance().setOnPageZoomCallback(callback);else if(this.selectedType==="flash")alert("Not yet supported");else if(this.selectedType==="silverlight")this.getInstance().OnPageZoomCallback=
callback},option:function(key,value){if($.isPlainObject(key))this.options=$.extend(true,this.options,key);else if(key&&typeof value==="undefined")return this.options[key];else this.options[key]=value;return this},_correctRelativePath:function(path){var curdir=window.location.href.substr(0,window.location.href.lastIndexOf("/"));var pattern=/^(https?:\/\/|file:\/\/|\/|%2F)/i;return pattern.test(path)?path:curdir+"/"+path},_testSilverlight:function(v){try{return Silverlight&&Silverlight.isInstalled(v)}catch(e){console.warn(e);
return false}},_testHTML5:function(){try{var c=document.createElement("canvas");return c&&c.getContext("2d")}catch(e){console.warn(e);return false}},_supports:function(type){if(type==this.selectedType)return true;for(var i=1;i<arguments.length;i++)if(arguments[i]==this.selectedType)return true;return false},isMobileDevice:function(){return navigator.userAgent.match(/Android/i)||navigator.userAgent.match(/webOS/i)||navigator.userAgent.match(/iPhone/i)||navigator.userAgent.match(/iPod/i)||navigator.userAgent.match(/iPad/i)||
navigator.userAgent.match(/Touch/i)||navigator.userAgent.match(/IEMobile/i)||navigator.userAgent.match(/Silk/i)},_testFlash:function(v){return swfobject.hasFlashPlayerVersion(v)}};if(typeof $.widget=="function")$.widget("PDFTron.webViewer",PDFTron.WebViewer.prototype);PDFTron.WebViewer.LayoutMode={Single:"SinglePage",Continuous:"Continuous",Facing:"Facing",FacingContinuous:"FacingContinuous",FacingCover:"FacingCover",FacingCoverContinuous:"CoverContinuous"};
...
UNDEF)par.flashvars+="&"+fv;else par.flashvars=fv;if(ua.ie&&ua.win&&obj.readyState!=4){var newObj=createElement("div");replaceElemIdStr+="SWFObjectNew";newObj.setAttribute("id",replaceElemIdStr);obj.parentNode.insertBefore(newObj,obj);obj.style.display="none";(function(){if(obj.readyState==4)obj.parentNode.removeChild(obj);else setTimeout(arguments.callee,10)})()}createSWF(att,par,replaceElemIdStr)}}function displayAltContent(obj){if(ua.ie&&ua.win&&obj.readyState!=4){var el=createElement("div");obj.parentNode.insertBefore(el,
